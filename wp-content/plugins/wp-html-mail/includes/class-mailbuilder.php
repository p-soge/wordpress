...
final class Haet_Mail_Builder
{
    ...
    public function register_post_type(){        
        $labels = array(
            'name'                => __( 'custom emails', 'haet_mail' ),
            'singular_name'       => __( 'custom email', 'haet_mail' ),
            'add_new'             => __( 'Add New Custom Email', 'haet_mail' ),
            'add_new_item'        => __( 'Add New Custom Email', 'haet_mail' ),
            'edit_item'           => __( 'Edit Custom Email', 'haet_mail' ),
            'new_item'            => __( 'New Custom Email', 'haet_mail' ),
            'view_item'           => __( 'View Custom Email', 'haet_mail' ),
            'search_items'        => __( 'Search Custom Emails', 'haet_mail' ),
            'not_found'           => __( 'No Custom Emails found', 'haet_mail' ),
            'not_found_in_trash'  => __( 'No Custom Emails found in Trash', 'haet_mail' ),
            'parent_item_colon'   => __( 'Parent Custom Email:', 'haet_mail' ),
            'menu_name'           => __( 'Custom Emails', 'haet_mail' ),
        );
    ...
    public function setup_meta_boxes(){
        add_meta_box( 'header', __( 'Message', 'haet_mail' ),
                        array($this,'render_header_meta_box'), 'wphtmlmail_mail', 'normal', 'high' );
        add_meta_box( 'subject', __( 'Email Subject', 'haet_mail' ),
                        array($this,'render_subject_meta_box'), 'wphtmlmail_mail', 'normal', 'high' );
        add_meta_box( 'mailbuilder', __( 'Email Content Builder', 'haet_mail' ),
    ...
    private function print_add_content_button(){
        ...
        <div class="mb-add-wrap">
            <a class="button button-primary button-large mb-add" href="#">
                <?php _e('Add Content Element','haet_mail'); ?>
            </a> 
        </div>
        <?php
        global $post;
    ...
    public function get_email_post_id( $email_name ){
        ...
        if( !get_post( apply_filters( 'wpml_object_id',  $options['email_post_ids'][$email_name], 'wphtmlmail_mail', FALSE ) ) )
            $options['email_post_ids'][$email_name] = $this->create_email( $email_name );            

        update_option('haet_mail_options', $options );
        return apply_filters( 'wpml_object_id',  $options['email_post_ids'][$email_name], 'wphtmlmail_mail', FALSE );
    }
